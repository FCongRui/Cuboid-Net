...
class RDN(nn.Module):
    def __init__(self, args,flag):
       ...
       #######32_64#######
        ...
        self.Conv = nn.Conv2d(G, G, kSize, padding=(kSize - 1) // 2, stride=1)


        self.upFrame = nn.Sequential(
            nn.ConvTranspose3d(G,G,kernel_size=self.K,stride=S,padding=P,output_padding=OP),
            nn.LeakyReLU(0.2),
    ...
    def forward(self, data):
        ...
        if self.flag == 'SAI':
           y1 = torch.zeros(b, 64, h * 4, w * 4, n).cuda()
           for j in range(n):
               x = data[:, :, :, j]
               ...
               y1[:, :, :, :, j] = x
           y1 = y1.permute([0,1,4,2,3])
           y = self.upFrame(y1)    ##帧率提升
           del y1
           return  y

        if self.flag == 'EPI_1':
            y2 = torch.zeros(b, 64, h, w * 4, n * 2).cuda()
            for j in range(h):
                x = data[:, j, :, :]  # w,n 共 h个
                x = x.unsqueeze(1)
                ...
                y2[:, :, j, :, :] = x
            y2 = y2.permute([0, 1, 4, 2, 3])
            y = self.upFrame(y2)  ##帧率提升
            del y2
            return y

        if self.flag == 'EPI_2':
            y3 = torch.zeros(b, 64, h * 4, w, n * 2).cuda()
            for j in range(w):
                x = data[:, :, j, :]  # h,n共w个
                x = x.unsqueeze(1)
...
class ARCNN(nn.Module):
    ...
    def forward(self, data):
        ...
        b, h, w, a = data.shape
        out = torch.zeros(b, 1, h, w, a).cuda()
        for i in range(a):
            x = data[:, :, :, i]  # h,w 共n个
            x = x.unsqueeze(1)
            x = self.base(x)
