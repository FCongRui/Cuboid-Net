...
class Trainer(object):
    ...
    def train(self,epoch):
        ...
        f = open("Loss_Enhanced_video_vimeoAll_4_2.txt", "a")
    ...
    def video_test(self):
            f = open("PSNR_SSIM_video_vimeo_detail_3to5.txt", "a")
            f1 = open("PSNR_SSIM_vimeo_SR_3to5.txt", "a")
            f2 = open("PSNR_SSIM_vimeo_VFI_3to5.txt", "a")
            ...
            with torch.no_grad():
                for batch_num, (data, target,Imgname) in enumerate(self.testing_loader):
                    ...
                    file = Imgname[0][-14:-4] + "\t"
                    print(file)
                    savepath = "./Save_video_vimeo_3to5/" + file + "/"
                    isExists = os.path.exists(savepath)
                    if not isExists:
                        os.makedirs(savepath)
